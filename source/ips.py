...
def test():
    ...
    if g == m:
        print(
            "Patch applied successfully, generated file is identical to what was expected."
        )
    else:
    ...
    print(
        createPatch(testDir + "original", testDir + "modified", testDir + "patch_gzip")
    )

    # try to apply it
    print("applyPatch() returns :")
    print(
        applyPatch(testDir + "original", testDir + "patch_gzip", testDir + "generated")
    )
...
def main():
    ...
    if len(sys.argv) != 5:
        print(
            "Error: ips.py takes 1 or 4 arguments, "
            + str(len(sys.argv) - 1)
            + " given."
        )
        return False
